				...
				<table width="100%" cellspacing="0" cellpadding="0">
					...
					<tr class="paging" style="text-align: center; font-size: 22px; padding-bottom: 30px;">
						<td>
							...
							<!--########## 이전 페이지 링크 출력 ############ -->
					     	<c:if test="${ curPage > 2 }">
								<a href="Diction_Order?initial=${initial}&curPage=${ 1 }">[1 페이지로]</a>
			 		    	</c:if> 
			 		    	<c:if test="${ curPage > 1 }">
								<a href="Diction_Order?initial=${initial}&curPage=${ curPage - 1 }">[이전 페이지]</a>
			 		    	</c:if> 
					     	 <c:if test="${ curPage > 10 }">
								<a href="Diction_Order?initial=${initial}&curPage=${ curPage - 10}">[이전 10 페이지]</a>
			 		    	</c:if> 
							...
							<!--########## 다음 페이지 링크 출력 ############ -->
							 <c:if test="${ curPage < totalPage  }">
								<a href="Diction_Order?initial=${initial}&curPage=${ curPage + 1}">[다음 페이지]</a>
							<c:if test="${ totalPage > 10  }">
								<a href="Diction_Order?initial=${initial}&curPage=${ curPage + 10}">[다음 10 페이지]</a>
							</c:if>
								<a href="Diction_Order?initial=${initial}&curPage=${ totalPage }">[마지막 페이지]</a>
							</c:if>
						</td>
					</tr> <!-- end paging -->
				</table>
