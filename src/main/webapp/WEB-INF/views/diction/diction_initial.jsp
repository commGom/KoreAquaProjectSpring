	...
	<main id="main"> <!-- ======= Breadcrumbs ======= -->
		...
		<section class="inner-page">
			...
			<table width="100%" cellspacing="0" cellpadding="0">
				...
				<tr>
					<td>
						<table align="center" width="40%" cellspacing="0" cellpadding="0" bDiction_Order="0">
							<tr>
								<td height="5"></td>
							</tr>
							<tr>
					...
					<tr class="paging" style="text-align: center; font-size: 22px; padding-bottom: 30px;">
						<td>
							...
							<!--########## 이전 페이지 링크 출력 ############ -->
							<c:if test="${ curPage > 2 }">
								<a href="Diction_Initial?searchValue=${searchValue}&curPage=${ 1 }">[1 페이지로]</a>
							</c:if>
							<c:if test="${ curPage > 1 }">
								<a href="Diction_Initial?searchValue=${searchValue}&curPage=${ curPage - 1 }">[이전 페이지]</a>
							</c:if>
					     	 <c:if test="${ curPage > 10 }">
								<a href="Diction_Initial?searchValue=${searchValue}&curPage=${ curPage - 10}">[이전 10 페이지]</a>
					    	</c:if> 
								...
								<c:if test="${ counter <= totalPage }">
									<c:choose>
										<c:when test="${ counter == curPage }">
											<a class="curPage" href="Diction_Initial?searchValue=${searchValue}&curPage=${counter }">[${counter }]</a>
										</c:when>
										<c:when test="${ counter != curPage }">
											<a href="Diction_Initial?searchValue=${searchValue}&curPage=${counter }">[${counter }]</a>
										</c:when> 
									</c:choose>
								</c:if>
							</c:forEach> 
							...
							<c:if test="${ curPage < totalPage  }">
								<a href="Diction_Initial?searchValue=${searchValue}&curPage=${ curPage + 1}">[다음 페이지]</a>
							<c:if test="${ totalPage > 10  }">
								<a href="Diction_Initial?searchValue=${searchValue}&curPage=${ curPage + 10}">[다음 10 페이지]</a>
							</c:if>
								<a href="Diction_Initial?searchValue=${searchValue}&curPage=${ totalPage }">[마지막 페이지]</a>
							</c:if>
						</td>
					</tr> <!-- end paging -->
				</table>
