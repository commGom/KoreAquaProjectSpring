	...
	<!-- ======= Breadcrumbs ======= -->
		...
		<ul> 
			<li class="page">
				...
				<!--########## 이전 페이지 링크 출력 ############ -->
				<c:if test="${ curPage > 2 }"> 
					<a href="Course?curPage=${ 1 }">[1 페이지로 ]</a>&nbsp;
				</c:if> 
				<c:if test="${ curPage > 10 }">
					<a href="Course?curPage=${ curPage - 10 }">[이전 10 페이지 ]</a>&nbsp;
				 </c:if> 
				<c:if test="${ curPage > 1 }">
					<a href="Course?curPage=${ curPage - 1 }">[이전 페이지 ]</a>&nbsp;
				</c:if> 
				...
				<c:if test="${ counter <= totalPage }">
					<c:choose>
						<c:when test="${ counter == curPage }">
							<a class="curPage" href="Course?curPage=${ curPage }">[${counter}]</a>&nbsp;
						</c:when>
					<c:when test="${ counter != curPage }">
							<a href="Course?curPage=${counter}">[${counter}]</a>&nbsp;
					</c:when>
					</c:choose>
				</c:if> 
				</c:forEach> 
				...
				<c:if test="${ curPage < totalPage  }">
					<a href="Course?curPage=${ curPage+1  }">[다음 페이지]</a>&nbsp;
				<c:if test="${ totalPage > 10  }">
					<a href="Course?curPage=${ curPage+10 }">[다음 10 페이지]</a>&nbsp;
				</c:if> 
					<a href="Course?curPage=${ totalPage  }">[마지막 페이지]</a>
				</c:if>
			</li>
		</ul> <!-- end paging -->
	</main>
